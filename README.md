...
2. Pls check `~/.config/i3/autostart/i3config` . There's changes in keyboard layout and resolution.
...
- [x] Add enviroment test with docker
- [ ] Uninstall and back config(Partial: some folders still remain.)
- [ ] Make scripts to generate configs (i3blocks, vim, etc)(Partial) 
- [x] check backup in stow installation
...
2. Edit `~/bin/dotfiles_env`, very important, file to put your repository url, branch name, email, user name and path where dotfiles will be installed and check folders and files(comment if you will not use)

3. `$ chmod +x bin/install` Execute `$ bash bin/install` script.

<br>

> Your default configs will be move to `~/.config-bkp/original`
...
> *Read and edit it first for your purpose*, it will create some folders and copy config files, atention to Dirs and files variable in bin/dotfiles_env.
...
1. Clone this repo in another local.(here the default is `~/.config/dotfiles/` )
2. Get to the correspondent branch by `$ git switch [bare or stow]` 
