...
@test "Test script if folder path contains spaces" {    
    cd "${BATS_TMPDIR}" || return
    mkdir -p "${BATS_TMPDIR}/pas ta/dir-es pa co" || return
    touch "${BATS_TMPDIR}/pas ta/file-ar qui vo" || return
    #cd "${BATS_TMPDIR}/$pathTo" || return
    echo "BEFORE ###>>> namestd"
    find "${BATS_TMPDIR}" -mindepth 1 |sort
    ...
    #[ -d "${BATS_TMPDIR}/pas_ta/dir-es_pa_co" ]
    #lista de arquivos e pastas não atualiza durante a execuçao do script
    #nesse caso para o teste funcionar precisaria de atualizar a lista 
    #de pastas durante a execução
    [ -f "${BATS_TMPDIR}/pas_ta/file-ar_qui_vo" ]
    cd "${BATS_TMPDIR}" || return
}
