...
@test "Repo I3blocks Config Is Valid" {
    ...
    run timeout --preserve-status 4.5s i3blocks -c $dotfiles_dir/.config/i3/i3blocks.conf
    [ "$status" -eq 0 ]
}
@test "Repo I3blocks_v Config Is Valid" {
    ...
    run timeout --preserve-status 4.5s i3blocks -c $dotfiles_dir/.config/i3/i3blocks_v.conf
    [ "$status" -eq 0 ]
}
