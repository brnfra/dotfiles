...
testLocalExistBinFiles() {
    ...
    assertTrue "${LINENO}:${red}${bold}[FAIL]${reset} foldering media by date not found.    " "[ -f $HOME/bin/filesByDate ]"
    assertTrue "${LINENO}:${red}${bold}[FAIL]${reset} foldering files by type not found.    " "[ -f $HOME/bin/filesByType ]"
